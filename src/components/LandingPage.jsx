...
import {
    ...
    ArrowLeft, Mail, Eye, EyeOff, Loader2, Chrome, X
} from 'lucide-react';
        ...
        <div className="fixed inset-0 z-50 flex items-end bg-black/40 backdrop-blur-sm">
            ...
            >
                <div className="p-6 space-y-6">
                    ...
                    {/* Tab Toggle */}
                    <div className="flex gap-2 p-1 bg-zinc-100 dark:bg-[#2A2A2A] rounded-lg">
                        {['login', 'signup'].map(t => (
                            <button
                                key={t}
                                onClick={() => setTab(t)}
                                className={`flex-1 py-2 px-3 rounded-md font-semibold text-xs transition-all ${
                                    tab === t
                                        ? 'bg-white dark:bg-[#1A1A1A] text-[#0057FF] dark:text-[#D4F000] shadow-sm'
                    ...
                    <div className="space-y-2">
                        <label className="text-xs font-bold text-[#0D0D0D] dark:text-white uppercase tracking-widest">Email</label>
                        <div className="relative">
                            <Mail size={16} className="absolute left-3 top-1/2 -translate-y-1/2 text-[#6B6B6B] dark:text-[#A0A0A0]" />
                            <input
                                type="email"
                                placeholder={emailPlaceholder}
                                className="w-full pl-10 pr-4 py-3 rounded-lg bg-zinc-50 dark:bg-[#2A2A2A] border border-zinc-200 dark:border-[#333333] text-[#0D0D0D] dark:text-white text-sm focus:outline-none focus:ring-2 focus:ring-[#0057FF] dark:focus:ring-[#D4F000]"
                            />
                        </div>
                    </div>
                    ...
                    <button
                        onClick={() => onGoogleLogin(role)}
                        disabled={loading}
                        className="w-full py-3 rounded-lg font-bold text-sm text-white transition-all hover:scale-[1.01] active:scale-[0.99] hover:shadow-lg disabled:opacity-60 disabled:cursor-not-allowed"
                        style={{ background: `linear-gradient(135deg, ${accentLight}, #0057FF)` }}
                    >
                        {loading
                            ? <Loader2 size={16} className="animate-spin mx-auto" />
